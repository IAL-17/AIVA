...
let isAwake = false; // Awake flag

function startRecognition() {
  recognition.start();
  console.log("Listening...");
  statusEl.innerText = "Waiting for 'Hey AIVA'... 🎙️";
}
...
recognition.onresult = (event) => {
  ...
  if (!isAwake) {
    if (transcript.includes('hey aiva') || transcript.includes('hi aiva') || transcript.includes('aiva')) {
      isAwake = true;
      statusEl.innerText = "Listening to you now... 🎧";
...
function handleCommand(command) {
  ...
  if (command.includes("how are you")) {
    ...
    response = "Going back to standby mode.";
    isAwake = false;
    statusEl.innerText = "Waiting for 'Hey AIVA'... 🎙️";
  } else {
